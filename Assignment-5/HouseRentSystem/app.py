...
app.config['DATABASE'] = 'rental_management.db'  # Default production database
...
@app.route('/upload_chat_pdf/<int:property_id>/<tenant_phone>', methods=['POST'])
def upload_chat_pdf(property_id, tenant_phone):
    if 'user_type' not in session:
        flash("Please log in to send messages.", "error")
        return redirect(url_for('login'))

    landlord_phone = session.get('phone') if session['user_type'] == 'landlord' else tenant_phone
    sender = session.get('username')
    ...
    if pdf_file and pdf_file.filename.endswith('.pdf'):
        ...
        pdf_file.save(os.path.join(app.config['LEASE_UPLOAD_FOLDER'], pdf_filename))

    # Save message and PDF reference in the database
    with sqlite3.connect('rental_management.db') as conn:
        cursor = conn.cursor()
        cursor.execute('''
    ...
    return redirect(url_for('chat_detail', property_id=property_id, tenant_phone=tenant_phone))
...
@app.route('/upload_chat_t_pdf/<int:property_id>/<landlord_phone>', methods=['POST'])
def upload_chat_t_pdf(property_id, landlord_phone):
    if 'user_type' not in session:
        flash("Please log in to send messages.", "error")
        return redirect(url_for('login'))

    tenant_phone = session.get('phone') if session['user_type'] == 'tenant' else landlord_phone
    sender = session.get('username')
    message_content = request.form.get('message')
    pdf_file = request.files.get('pdf')
    ...
    flash("Message sent successfully!", "success")
    return redirect(url_for('mychat_detail_t', property_id=property_id, landlord_phone=landlord_phone))
